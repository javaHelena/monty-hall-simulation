...
public class Game {
    ...
    final static int NUMBER_OF_BOXES = 3;
    private Map<String, Box> availableBoxes = new HashMap<String, Box>(NUMBER_OF_BOXES);
    ...
    private void initBoxes() {
        ...
        List<Box> boxes = new ArrayList<Box>(NUMBER_OF_BOXES);
    ...
    private boolean findLastBoxContent() throws MontyHallGameException {
        Box remaningBox = null;
        Set<String> keys = availableBoxes.keySet();
        if(availableBoxes.keySet().size() != 1){
            throw new MontyHallGameException("There are too many boxes in the game. Start over. ");
        } else {
            for (String key : keys) {
                remaningBox = availableBoxes.get(key);
            }
        }
        return remaningBox.containsPrize;
    }
